<div align="center">
  <a href="https://github.com/Earthzeta-Network/ai-agent-template-openai">
  ...
  <p align="center">
    Host your AI Agent Contract on Earthzeta's decentralized serverless cloud.
    <br />
    <a href="https://github.com/Earthzeta-Network/ai-agent-template-openai"><strong>Explore the docs »</strong></a>
    <br />
    <br />
...
The OpenAI AI Agent template is a **MINIMAL** template to build an AI Agent that can be hosted on Earthzeta Network's decentralized hosting protocol. Unlike Vercel or other FaaS, it allows you to publish your AI Agent compiled code to IPFS and hosts it on a fully decentralized FaaS cloud with the following benefits:

- 💨 Ship Fast: Build and ship with familiar toolchain in minutes
- ⛑️ Secure: Execution guarded by rock solid TEE / Intel SGX
- 🔒 Private: Host API keys and user privacy at ease
- 💎 Unstoppable: Powered by IPFS and Earthzeta's 35k+ decentralized TEE workers
...
> AI Agent deployed at: https://agents.Earthzeta.network/ipfs/QmQu9AmBL13tyGpxgg5ASt96WQ669p63rnJRWiAo9st8ns/0
>
...
```shell
curl -F file=@./dist/index.js https://agents.Earthzeta.network/ipfs 
```
</details>

### Access the Published AI Agent

Once published, your AI Agent is available at the URL: `https://agents.Earthzeta.network/ipfs/<your-cid>`. You can get it from the "Publish to IPFS" step.

You can test it with `curl`.

```bash
curl https://agents.Earthzeta.network/ipfs/<your-cid>
```
...
**Open terminal**
Use `curl` to `POST` your secrets to `https://agents.Earthzeta.network/vaults`. Replace `IPFS_CID` with the CID to the compile JS code in IPFS, and replace `<OPENAI_API_KEY>` with your OpenAI API key. Note that you can name the secret field name something other than `openaiApiKey`, but you will need to access the key in your `index.ts` file with the syntax `req.secret?.<your-secret-field-name> as string`

The command will look like this:
```shell
curl https://agents.Earthzeta.network/vaults -H 'Content-Type: application/json' -d '{"cid": "IPFS_CID", "data": {"openaiApiKey": "<OPENAI_API_KEY>"}}'
# Output:
...
```shell
curl https://agents.Earthzeta.network/vaults/<key>/<token>
```
...
```text
https://agents.Earthzeta.network/ipfs/<cid>?key=<key>
```

Example:
https://agents.earthzeta.network/ipfs/QmQu9AmBL13tyGpxgg5ASt96WQ669p63rnJRWiAo9st8ns/0?key=c0c0105ba56276cd&chatQuery=When%20did%20humans%20land%20on%20the%20moon
...
```
For example, if you visit https://agents.earthzeta.network/ipfs/QmQu9AmBL13tyGpxgg5ASt96WQ669p63rnJRWiAo9st8ns/0?key=c0c0105ba56276cd&chatQuery=When%20did%20humans%20land%20on%20the%20moon, the `chatQuery` value will be "When did humans land on the moon". Note that `queries` can use any field name - `chatQuery` is just an example. Make sure to update your `index.ts` file logic to match whatever field name you choose to use.
...
<ul>
  ...
  <li>Security: The code is executed in the decentralized TEE network running on Earthzeta Network. It runs code inside a secure blackbox (called enclave) created by the CPU. It generates cryptographic proofs verifiable on Earthzeta blockchain. It proves that the hosted code is exactly the one you deployed.</li>
  <li>Privacy: You can safely put secrets like API keys or user privacy on Earthzeta Network. The code runs inside TEE hardware blackboxs. The memory of the program is fully encrypted by the TEE. It blocks any unauthorized access to your data.</li>
  <li>Learn more at <a href="https://Earthzeta.network">Earthzeta Network Homepage</a></li>
</details>
